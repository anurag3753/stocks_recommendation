...
class EveningStar( Candle ):
    ...
    def run():
        ...
        if self.uptrend and is_bullish(ppo, ppc) and gap_up_opening(po, ppc) and (sp_top or doji) \
            and gap_down_opening(self.o, pc) and (self.c < ppo) and is_bearish(self.o, self.c):
            self.candle = True
            self.trade_setting["action"] = "sell"
            self.trade_setting["buy"] = self.c
            self.trade_setting["stoploss"] = min(self.h, ph, pph)
            self.trade_setting["candle"] = "evening_star"
            self.trade_setting["target"] = ""
            self.trade_setting["info"]   = general_info("evening_star")
